...
class ToursListTest extends TestCase
{
    ...
    public function test_sorts_by_price_correctly()
    {
        $travel = Travel::factory()->create();
        ...
        $response->assertStatus(200);
        $response->assertJsonPath('data.0.id', $cheaperLaterTour->id);
        $response->assertJsonPath('data.1.id', $cheaperEarlierTour->id);
        $response->assertJsonPath('data.2.id', $expensiveTour->id);

    }

    public function test_filters_by_price_correctly()
    {
        $travel = Travel::factory()->create();
        ...
        $expensiceTour = Tour::factory()->create([
            'travel_id' => $travel->id,
            'price' => 200,
        ]);
        ...
        $response->assertJsonFragment(['id' => $expensiceTour->id]);
        ...
        $response->assertJsonCount(1, 'data');
        $response->assertJsonMissing(['id' => $expensiceTour->id]);
        $response->assertJsonFragment(['id' => $cheaperTour->id]);
    ...
    public function test_filters_by_date_correctly()
    {
        $travel = Travel::factory()->create();
        $earlierTour = Tour::factory()->create([
            'travel_id' => $travel->id,
